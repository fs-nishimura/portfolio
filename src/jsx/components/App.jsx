...
const CanvasUtils = require("babel!../../js/classes/utils/canvasUtils.js").default;
...
class App extends React.Component {
  ...
  render() {
    let isTop = this.props.pathname.length==1?true:false;
    return (
      <div id="wrapAll">
        <div className="ribbon"><span>Beta</span></div>
        <div className="contents">
          <h1 className="logo"><Link to="/" key="/">nnishimura<span className="dot">.</span>io</Link></h1>
...
window.addEventListener("mousemove",function(e){
  let ratioX = (Data.winW/2 - e.pageX) * 0.03;
  let ratioY = (Data.winH/2 - e.pageY) * 0.01;
  let timer;

  for (let i = 0; i < Data.numShapes; i++) {
    let currentX = Data.shapes[i].x;
    let index = i;
